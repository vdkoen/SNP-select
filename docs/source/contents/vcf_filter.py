...
It has also the option to filter pairs that are not similar in genotypes only selecting stable variants within a variaty

Script can use a Freebayes and Samtools variant files. Make sure to use samtools variant caller with --output-tags DP,AD,ADF,ADR,SP
...
def check_pairs(record, pairs, vcf_type):
    ...
    If not the same remove all the calls for those pairs, so do not consider those in downstream analysis
    """
...
def sample_het(record, vcf_type):
    """
    Filter sample heterozygote calls, you expect around 0.50/0.50 reads mapped if it is a heterozygote call
    But does not always happen so remove those calls when they not in the 0.30/0.70 ratio.
    """
    for sample in range(0, len(record.samples)):
        if record.samples[sample].called:
            if record.samples[sample].is_het:
...
def genotype_depth(record, call_rate):
    """Filter records if the dont have enough genotypes called, so these sides are not very informative"""
