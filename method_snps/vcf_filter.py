...
"""
VCF file filter, filter is designed for GBS data. It focuses on removing non informative sides.
It has also the option to filter pairs that are not similar in genotypes only selecting
stable variants within a variaty

Script can use a Freebayes and Samtools variant files. Make sure to use samtools variant caller
with --output-tags DP,AD,ADF,ADR,SP
Output is a filtered vcf file containing only the variants that match the criteria.
"""
...
def parse_options():
    ...
    parser.add_option("-q", "--qual", dest="record_qual",
                      help="Minimal quality a record needs to have to pass the filtering",
                      metavar="QUAL", type="int",
                      default="20")

    parser.add_option("-d", "--depth", dest="record_depth",
                      help="Minimal amount of reads mapped before calling a variant at that "
                           "location",
                      metavar="DEPTH",
                      type="int", default="30")

    parser.add_option("-s", "--sample_depth", dest="sample_depth",
                      help="Minimal depth per sample", metavar="SAMPLE_DEPTH", type="int",
                      default="3")

    parser.add_option("-r", "--call", dest="call_rate",
                      help="Amount of genotypes which need to be present, before including in "
                           "downstream analysis",
    ...
    parser.add_option("-f", "--fraction_het", dest="fraction_het",
                      help="Fraction of heterozygous in a record before filtering it, the higher "
                           "it is the larger the chance of not removing faulty heterozygous "
                           "records. But if its to low it could remove "
...
# Since pyvcf makes a Calldata object which is not assignable,
# we make a new one with the correct information.

def edit_sample(sample, vcf_type):
    """Edit sample in a Record, if sample does not match criteria the information is removed"""
...
def filter_sample_depth(record, sample_depth, vcf_type):
    """
    Sample depth filter, filtering on minimal sample depth,
    but also on AD since DP is not always a good representation
    """
    for sample in range(0, len(record.samples)):
        try:
            ...
            # DP somethimes is not a good representation.
...
def filter_site(record, fraction_het):
    """
    This function checks if a record is trust worthy based on the heterozygous calls,
    you can expect those to be around 0.5/0.5 for diploide samples. If this ratio is off for all
    the calls combined, it might indicate a faulty side and therefore those sides are filtered
    out and excluded.
    """
    het = 0
    hom = 0
    ...
    for sample in range(0, len(record.samples)):
        ...
        if record.samples[sample].called is False:
            pass

        else:
            # if it is a heterozygote with more then 2 different bases, we don't look at it yet.
            # let the record pass can also let it fail... not sure what to do.
            if len(record.samples[sample]["AD"]) > 2:
                ...
                elif record.samples[sample].is_het is False:
                    hom += 1
    ...
    try:
        ...
        if het / (het + hom) > fraction_het:
            for sample in range(0, len(record.samples)):
                ...
                if record.samples[sample].called:
                    if record.samples[sample].is_het:
                        some_value += max(record.samples[sample]["AD"]) / sum(
                            record.samples[sample]["AD"])
        ...
        # return false since something/0 is not possible.
        # meaning that something is called but not a het or hom. can not trust these sites.
        return False


def check_pairs(record, pairs, vcf_type):
    """
    Filter pairs, the genotypes have to be the same between all the pairs.
    If not the same remove all the calls for those pairs,
    so do not consider those in downstream analysis
    """
    ...
    for pair in pairs:
        ...
        if allow_record:
            if len(set(pair_hits)) > 1:
                for sample in pair:
                    record.samples[record._sample_indexes[sample]] = edit_sample(
                        record.samples[record._sample_indexes[sample]], vcf_type)
...
def filter_qual(record, minimal_qual):
    """Standard record Quality filter"""
    try:
...
def sample_het(record, vcf_type):
    """
    Filter sample heterozygote calls, you expect around 0.50/0.50 reads mapped if it is a
    heterozygote call But does not always happen so remove those calls when they not in the
    0.30/0.70 ratio.
    """
    for sample in range(0, len(record.samples)):
        if record.samples[sample].called:
            if record.samples[sample].is_het:
                try:
                    if not 0.3 <= max(record.samples[sample]["AD"]) / sum(
                            record.samples[sample]["AD"]) <= 0.7:
                        record.samples[sample] = edit_sample(record.samples[sample], vcf_type)
                        record.samples[sample].called = False
                except ZeroDivisionError:
                    pass
...
def genotype_depth(record, call_rate):
    """
    Filter records if the dont have enough genotypes called, so these sides are not very
    informative
    """
